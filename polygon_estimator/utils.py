...
class EarlyStopping:
    """
    Validation skoru (Dice/IoU) iyileşmediğinde eğitimi durdurur.
    'Higher is Better' (Skor artmalı) mantığıyla çalışır.
    """
    def __init__(self, patience=7, verbose=False, delta=0, path='checkpoint.pt'):
        """
        Args:
            patience (int): İyileşme beklenecek epoch sayısı.
            verbose (bool): Mesaj yazdırılsın mı?
            delta (float): İyileşme olarak kabul edilecek minimum değişim.
        ...
        self.val_score_max = -np.inf # Maksimum skoru takip edeceğiz
        self.delta = delta
        self.path = path

    def __call__(self, val_score, model):
        # Skorun kendisine bakıyoruz (Higher is Better mantığı)
        score = val_score
    ...
    def save_checkpoint(self, val_score, model):
        '''Validation skoru arttığında modeli kaydeder.'''
        if self.verbose:
            print(f'Validation score improved ({self.val_score_max:.4f} --> {val_score:.4f}).  Saving model ...')
